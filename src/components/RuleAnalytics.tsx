...
export const RuleAnalytics: React.FC<RuleAnalyticsProps> = ({
  ...
  const triggersOverTimeData = useMemo(() => {
    ...
    return days.map(day => {
      ...
      rules.forEach(rule => {
        ...
        dayData[rule.name] = Math.floor(Math.random() * rule.statistics.timesTriggered / 7);
      });
      return dayData;
    });
  }, [rules]);
  ...
  const ruleEffectivenessData = useMemo(() => {
    return rules.map(rule => ({
      ...
      triggered: rule.statistics.timesTriggered,
      alertsCreated: rule.statistics.alertsCreated,
      efficiency: rule.statistics.alertsCreated / Math.max(rule.statistics.timesTriggered, 1),
    }));
  }, [rules]);

  const executionTimeData = useMemo(() => {
    return rules.map(rule => ({
      name: rule.name.length > 12 ? rule.name.substring(0, 12) + '...' : rule.name,
      ...
      <Card variant="elevated">
        ...
        <CardContent>
          <div style={{
            ...
            <div>
              Create rules to view performance analytics.
            </div>
          </div>
        </CardContent>
      </Card>
          ...
          <ResponsiveContainer width="100%" height={height}>
            <LineChart data={triggersOverTimeData}>
              ...
              <YAxis 
                stroke={theme.colors.textMuted}
                fontSize={12}
                label={{ value: 'Triggers', angle: -90, position: 'insideLeft' }}
              />
              <Tooltip content={<CustomTooltip />} />
              <Legend />
              {rules.map((rule, index) => (
                <Line
                  ...
                  activeDot={{ r: 6, stroke: getRuleColor(index), strokeWidth: 2 }}
                />
              ))}
            </LineChart>
          </ResponsiveContainer>
          ...
          <ResponsiveContainer width="100%" height={height}>
            <PieChart>
              <Pie
                ...
                labelLine={false}
              >
                {ruleEffectivenessData.map((entry, index) => (
          ...
          <div style={{
            ...
            <div style={{ textAlign: 'center' }}>
              <div style={{
                ...
                {Math.round(rules.reduce((sum, r) => sum + r.statistics.successRate, 0) / rules.length)}%
              </div>
              <div style={{
                fontSize: theme.typography.fontSize.sm,
                color: theme.colors.textMuted,
